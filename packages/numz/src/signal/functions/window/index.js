...
export const tukey_window=(N,alpha=0.5,ArrayType=Float64Array)=>{
  const w=new ArrayType(N)
  if(N===1){w[0]=1;return w}
  if(alpha<=0){w.fill(1);return w}
  if(alpha>=1) alpha=1
  const edge=alpha*(N-1)/2
  const denominator=alpha*(N-1)
  for(let n=0;n<N;n++){
    if(n<edge)
      w[n]=0.5*(1+cos(PI*(2*n/denominator-1)))
    else if(n>N-1-edge)
      w[n]=0.5*(1+cos(PI*(2*n/denominator-2/alpha+1)))
    else
      w[n]=1
  }
  return w
